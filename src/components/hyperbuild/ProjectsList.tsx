...
export function ProjectsList({ projectType, onNewProject }: ProjectsListProps) {
  ...
  const { data: projects, isLoading } = useQuery({
    queryKey: ['projects', workspace?.id, projectType],
    queryFn: async () => {
      ...
      return (data || []).filter(project => {
        const isSite = project.template_id?.includes('landing') || 
                      project.template_id?.includes('site') || 
                      project.template_id?.includes('page') ||
                      project.template_id?.startsWith('site-');
        return projectType === 'site' ? isSite : !isSite;
      });
    },
  ...
  const typeLabel = isApp ? 'Aplicativo' : 'Site';
  const emptyIcon = isApp ? Smartphone : Globe;
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-foreground">Projetos criados dentro da plataforma</h2>
          <p className="text-muted-foreground text-sm">
            Aqui ficam salvos todos os projetos que você criou para editar, atualizar, duplicar ou entregar ao cliente.
          </p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <Input
          placeholder="Buscar projetos criados..."
          value={searchTerm}
        ...
        <Card className="border-dashed">
          <CardContent className="p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">
              {searchTerm ? 'Nenhum projeto encontrado' : `Nenhum ${typeLabel.toLowerCase()} criado ainda.`}
            </h3>
            <p className="text-muted-foreground mb-4 max-w-sm">
              {searchTerm 
                ? 'Tente buscar com outros termos' 
                : `Crie seu primeiro ${typeLabel.toLowerCase()} para começar.`}
            </p>
            {!searchTerm && (
              <Button onClick={onNewProject} className="gap-2">
                <Plus className="h-4 w-4" />
                Criar Primeiro {typeLabel}
              </Button>
            )}
          </CardContent>
        </Card>
