...
export function RadarIdleAnimation({ isScanning = false, scanProgress = 0 }: { isScanning?: boolean; scanProgress?: number } = {}) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = 'hsla(260, 40%, 60%, 0.5)';
      ctx.fillText(isScanningRef.current ? 'ESCANEANDO...' : 'PROGRESS', barX, barY - 6);
      ...
      const progress = isScanningRef.current ? scanProgressRef.current / 100 : (Math.sin(frame * 0.015) + 1) / 2;
      ...
      ctx.fillText(isScanningRef.current ? 'Buscando empresas na região...' : 'Aguardando input de localização...', barX, barY + 16);
